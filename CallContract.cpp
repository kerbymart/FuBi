    ...
    std::vector<CallDiagnostic>& diagnostics)
{
    ...
    if (request.allowInternal && (!request.internalAuthorization || request.moduleSha256 != catalog.Module().sha256 || request.modulePath != catalog.Module().canonicalPath || request.moduleTimestamp != catalog.Module().timestamp || request.moduleImageSize != catalog.Module().imageSize || request.modulePreferredImageBase != catalog.Module().preferredImageBase || request.modulePdbGuid != catalog.Module().pdbGuid || request.modulePdbAge != catalog.Module().pdbAge)) Diagnostic(diagnostics, "module-identity-required", "module_identity", "internal calls require hash-pinned authorization for the complete catalog identity");
...
{
    output << "{\"schema_version\":" << request.schemaVersion << ",\"correlation_id\":"; Json(output, request.correlationId); output << ",\"selector\":"; Json(output, request.selector); output << ",\"module_sha256\":"; Json(output, request.moduleSha256); output << ",\"module_path\":"; Json(output, request.modulePath); output << ",\"module_timestamp\":" << request.moduleTimestamp << ",\"module_image_size\":" << request.moduleImageSize << ",\"module_preferred_image_base\":" << request.modulePreferredImageBase << ",\"module_pdb_guid\":"; Json(output,request.modulePdbGuid); output << ",\"module_pdb_age\":" << request.modulePdbAge << ",\"internal_authorization\":" << (request.internalAuthorization ? "true" : "false") << ",\"timeout_ms\":" << request.timeoutMs << ",\"allow_internal\":" << (request.allowInternal ? "true" : "false") << ",\"has_prototype_override\":" << (request.hasPrototypeOverride ? "true" : "false") << ",\"prototype_override\":"; if (request.hasPrototypeOverride) WritePrototype(output, request.prototypeOverride); else output << "null"; output << ",\"arguments\":[";
...
bool ParseCallResultJson(const std::string& document, CallResult& result, std::vector<CallDiagnostic>& diagnostics)
{
    ...
    if(pos!=std::string::npos && NextObject(document,pos,object)) { if(!TypeObject(object,result.returnType)) Diagnostic(diagnostics,"invalid-return-type","return_type","return type is malformed"); } else Diagnostic(diagnostics,"missing-field","return_type","return type is required");
    ...
    pos=document.find("\"prototype_used\":"); if(pos!=std::string::npos && NextObject(document,pos,object)) { if(!PrototypeObject(object,result.prototypeUsed)) Diagnostic(diagnostics,"invalid-prototype","prototype_used","prototype is malformed"); }

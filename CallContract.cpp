    ...
    std::vector<CallDiagnostic>& diagnostics)
{
    ...
    if (request.allowInternal && (request.authorizationProvenance != "profile:" + catalog.Module().sha256 || request.moduleSha256 != catalog.Module().sha256 || request.modulePath != catalog.Module().canonicalPath || request.moduleTimestamp != catalog.Module().timestamp || request.moduleImageSize != catalog.Module().imageSize || request.modulePreferredImageBase != catalog.Module().preferredImageBase || request.modulePdbGuid != catalog.Module().pdbGuid || request.modulePdbAge != catalog.Module().pdbAge)) Diagnostic(diagnostics, "module-identity-required", "module_identity", "internal calls require trusted profile provenance and complete catalog identity");
...
{
    output << "{\"schema_version\":" << request.schemaVersion << ",\"correlation_id\":"; Json(output, request.correlationId); output << ",\"selector\":"; Json(output, request.selector); output << ",\"module_sha256\":"; Json(output, request.moduleSha256); output << ",\"module_path\":"; Json(output, request.modulePath); output << ",\"module_timestamp\":" << request.moduleTimestamp << ",\"module_image_size\":" << request.moduleImageSize << ",\"module_preferred_image_base\":" << request.modulePreferredImageBase << ",\"module_pdb_guid\":"; Json(output,request.modulePdbGuid); output << ",\"module_pdb_age\":" << request.modulePdbAge << ",\"authorization_provenance\":"; Json(output,request.authorizationProvenance); output << ",\"timeout_ms\":" << request.timeoutMs << ",\"allow_internal\":" << (request.allowInternal ? "true" : "false") << ",\"has_prototype_override\":" << (request.hasPrototypeOverride ? "true" : "false") << ",\"prototype_override\":"; if (request.hasPrototypeOverride) WritePrototype(output, request.prototypeOverride); else output << "null"; output << ",\"arguments\":[";
